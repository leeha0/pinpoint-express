...
const IdGenerator = require('../../utils/sequence_number_singleton.js');

// change module.module.method() ->
// module/module.method() 
// as java api format class.method()
function formatApiInfo(apiInfo) {
    const index = apiInfo.lastIndexOf('.');
    const classStr = apiInfo.substring(0, index).replace(/\./g, '/');
    const methodStr = apiInfo.substring(index, apiInfo.length);
    return classStr + methodStr;
}

const MethodDescriptor = function MethodDescriptor() {
